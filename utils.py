...
def calculate_stay_probability(data: pd.DataFrame) -> pd.DataFrame:
    ...
    tmp_df['repeated_action_stage_1'] = tmp_df['action_stage_1'].shift(1) == tmp_df['action_stage_1']
    tmp_df['rewarded'] = tmp_df['reward_stage_2'] > 0
    ...
    results = tmp_df.groupby(['rewarded', 'common_transition'])['repeated_action_stage_1'].mean().reset_index()
    
    # rename columns for clarity
    results.rename(columns={'repeated_action_stage_1': 'Stay Probability', 'rewarded': 'Rewarded', 'common_transition': 'Common'}, inplace=True)
    ...
    results['Condition'] = results.apply(lambda row: conditions[(row['Rewarded'], row['Common'])], axis=1)
    
    # rounding the stay probabilities
    results['Stay Probability'] = results['Stay Probability'].apply(lambda x: np.round(x, 3))
...
def plot_stay_probability(data):
    ...
    y_limit_min = 0.5 if df['Stay Probability'].min() > 0.5 else df['Stay Probability'].min() - 0.1
